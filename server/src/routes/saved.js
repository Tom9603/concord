...
router.post('/', (req, res) => {
  const { content, attachment_url, author_name, source, remindInSeconds } = req.body || {};
  const remindAt = Number.isFinite(remindInSeconds) && remindInSeconds > 0 ? nowEpoch() + Math.floor(remindInSeconds) : null;
...
router.patch('/:id', (req, res) => {
  ...
  const { remindInSeconds } = req.body || {};
  const remindAt = Number.isFinite(remindInSeconds) && remindInSeconds > 0 ? nowEpoch() + Math.floor(remindInSeconds) : null;
